...
public class RWAnimationPlayer<AnimationLibraryType, ownerType>
    ...
    where ownerType : PhysicalObject
{
    ...
    private RWAnimation<ownerType> _currentAnimation;

    private int _waitingForAnimationEventIndex;
    /// <summary>
    /// The index of the animation event that is queued next.
    /// </summary>
    public int WaitingForAnimationEventIndex => _waitingForAnimationEventIndex;
    ...
    /// </summary>
    public RWAnimation<ownerType> CurrentAnimation => _currentAnimation;
    ...
    public RWAnimationPlayer(AnimationLibrary<AnimationLibraryType, ownerType> animationLibrary, ownerType owner)
    {
    ...
    {
        if (_currentAnimation.length != -1 && _animationTimer > _currentAnimation.length)
        {
